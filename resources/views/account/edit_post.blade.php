		...
		<section class="editor {{ $errors->has('content') ? 'has-error' : '' }}">
				...
				<div class="editorwrap">
					<section class="entry-markdown">
						...
						<section class="entry-markdown-content">
							<textarea id="entry-markdown" name="content" autofocus="true">{{ $raw_content }}</textarea>
						</section>
					</section>
			...
			<div class="editor-footer row">
				<div class="btn-group dropup pull-right">
					@if ($post->active)
						<button id="btn-submit-post" type="submit" class="btn btn-primary btn-sm">Đăng bài</button>
						<button id="btn-toggle-status" type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown">
				...
				<div id="sg-post-tags">
					<i class="fa fa-tags pull-left"></i> <input name="post_tags" id="tags" value="{{ implode(',', $post->relationTags->pluck('name')->toArray()) }}" />
				</div>
			</div>
		</section>
	</section>
...
@section("scripts")
  ...
  <script>
		$(function() {
			...
			var postLocal = JSON.parse(localStorage.getItem('edit'));

			$('.date-timer').appendDtpicker({
				'current' : '{{ date('Y-m-d H:i', $post->schedule_post) }}'
			});
			...
			fsdEditor.on('keyup', function() {
				...
				timing = setTimeout(function() {
					...
					if (postContent != '') {
						...
						localStorage.setItem('edit', JSON.stringify(postTemp));
			...
			//
			if (postLocal != null && postLocal.url == urlEdit) {
